...
def build_trade_key(trade: dict) -> str:
    ...
    premium = str(trade.get("total_premium") or trade.get("premium") or "")
    size = str(trade.get("total_size") or trade.get("size") or "")
...
def grade_signal(trade: dict):
    ...
    premium = parse_float(trade.get("total_premium") or trade.get("premium"))
    size = parse_int(trade.get("total_size") or trade.get("size"))
...
def ping():
    return jsonify({
        ...
        "sent_matches_count": len(sent_matches),
    }), 200
...
def monitor_loop():
    ...
    while True:
        try:
            ...
            print(f"Quiet Alpha bot error: {e}", flush=True)
